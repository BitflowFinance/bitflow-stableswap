...
const testArbitrage = (config: PoolConfig, attemptSize: number): { profitPercent: number; priceImpact: number; success: boolean } => {
    try {
        ...
        // Calculate profit and price impact
        ...
        const expectedStSTX = Math.floor(attemptSize * (STSTX_PRICE_USD / STX_PRICE_USD));
        const priceImpact = Math.abs((finalStSTX - expectedStSTX) / expectedStSTX) * 100;
                ...
                for (const amp of AMP_RANGES) {
                    for (const reversed of [false]) {
                        it(`Testing M:${midpoint.name} F:${factor.name} A:${amp.name} R:${reversed}`, () => {
                            ...
                            const score = (averageProfit * 2) + (averageImpact * 0.5) + (maxProfit * 3);
                            ...
                            console.log(`Score: ${colors.info(score.toFixed(4))} ${colors.info('(lower is better)')}`);

                            // Assertions
                            if (successCount > 0) {
                                expect(maxProfit).toBeLessThan(5); // Max profit should be less than 5%
                                expect(averageImpact).toBeLessThan(50); // Average price impact should be less than 50%
                            }
                        });
                    }
                }
            ...
            summary: {
                ...
                recommendations: [
                    ...
                    },
                    {
                        ...
                        config: sortedResults.reduce((a, b) => a.maxProfitPercent < b.maxProfitPercent ? a : b),
                        reason: "Lowest maximum profit potential for arbitrageurs"
                    },
                    {
