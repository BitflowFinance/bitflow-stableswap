...
(define-read-only (get-x (y-amount uint) (y-bal uint) (x-bal uint) (amp uint) (threshold uint))
  (let (
    ...
    (c-c (/ (* c-b current-d) (* amp-scaled MATH_NUM_2)))
...
(define-read-only (get-d (x-bal uint) (y-bal uint) (amp uint) (threshold uint))
  (get converged (fold fold-d-for-loop index-list {x-bal: x-bal, y-bal: y-bal, d: (+ x-bal y-bal), amp: (* amp MATH_NUM_2), threshold: threshold, converged: u0}))
)
...
(define-private (fold-d-for-loop (n uint) (static-data {x-bal: uint, y-bal: uint, d: uint, amp: uint, threshold: uint, converged: uint})) 
  (let (
    ...
    (new-numerator (* (+ (* current-amp current-s) (* MATH_NUM_2 new-d-partial)) current-d))
    (new-denominator (+ (* (- current-amp MATH_NUM_1) current-d) (* (+ MATH_NUM_2 MATH_NUM_1) new-d-partial)))
