...
(define-constant BPS_1 u10000)
(define-constant BPS_2 u10)
(define-constant BPS_3 u2)
(define-constant BPS_4 u1)
...
(define-public (get-dy
  ...
  (let (
    ...
    (x-amount-fees-protocol-scaled (/ (* x-amount-scaled protocol-fee) BPS_1))
    ...
    (updated-y-balance-scaled (get-y dx-scaled x-balance-scaled y-balance-scaled (* amplification-coefficient BPS_3) convergence-threshold))
    ...
    (asserts! (and (> x-amount u0) (< x-amount (* x-balance BPS_2))) ERR_INVALID_AMOUNT)
    (ok dy)
  )
)
...
(define-public (get-dx
  ...
  (let (
    ...
    (y-amount-fees-protocol-scaled (/ (* y-amount-scaled protocol-fee) BPS_1))
    ...
    (updated-x-balance-scaled (get-x dy-scaled y-balance-scaled x-balance-scaled (* amplification-coefficient BPS_3) convergence-threshold))
    ...
    (asserts! (is-eq (contract-of x-token-trait) x-token) ERR_INVALID_X_TOKEN)
    (asserts! (is-eq (contract-of y-token-trait) y-token) ERR_INVALID_Y_TOKEN)
    (asserts! (and (> y-amount u0) (< y-amount (* y-balance BPS_2))) ERR_INVALID_AMOUNT)
    (ok dx)
  )
)
...
(define-public (get-dlp
  ...
  (let (
    ...
    (d-a (get-d x-balance-scaled y-balance-scaled (* amplification-coefficient BPS_3) convergence-threshold))
    (d-b (get-d updated-x-balance-scaled updated-y-balance-scaled (* amplification-coefficient BPS_3) convergence-threshold))
    ...
    (ideal-x-amount-fee-liquidity-scaled (/ (* x-difference liquidity-fee) BPS_1))
    (ideal-y-amount-fee-liquidity-scaled (/ (* y-difference liquidity-fee) BPS_1))
    ...
    (updated-d (get-d updated-balance-x-post-fee-scaled updated-balance-y-post-fee-scaled (* amplification-coefficient BPS_3) convergence-threshold))
...
(define-read-only (get-x (y-amount uint) (y-bal uint) (x-bal uint) (ann uint) (threshold uint))
  (let (
    ...
    (c-a current-d)
    (c-b (/ (* c-a current-d) (* BPS_3 updated-y-balance)))
    (c-c (/ (* c-b current-d) (* ann BPS_3)))
...
(define-read-only (get-y (x-amount uint) (x-bal uint) (y-bal uint) (ann uint) (threshold uint))
  (let (
    ...
    (current-d (get-d x-bal y-bal ann threshold))
    (c-a current-d)
    (c-b (/ (* c-a current-d) (* BPS_3 updated-x-balance)))
    (c-c (/ (* c-b current-d) (* ann BPS_3)))
...
(define-public (set-x-fees (pool-trait <stableswap-pool-trait>) (protocol-fee uint) (provider-fee uint))
  (let (
    ...
    (begin
      ...
      (asserts! (is-valid-pool (get pool-id pool-data) (contract-of pool-trait)) ERR_INVALID_POOL)
      (asserts! (is-eq (get pool-created pool-data) true) ERR_POOL_NOT_CREATED)
      (asserts! (<= (+ protocol-fee provider-fee) BPS_1) ERR_INVALID_FEE)
...
(define-public (set-liquidity-fee (pool-trait <stableswap-pool-trait>) (fee uint))
  (let (
    ...
    (begin
      ...
      (asserts! (is-valid-pool (get pool-id pool-data) (contract-of pool-trait)) ERR_INVALID_POOL)
      (asserts! (is-eq (get pool-created pool-data) true) ERR_POOL_NOT_CREATED)
      (asserts! (<= fee BPS_1) ERR_INVALID_FEE)
...
;; should we mint minimum share to pool contract like xyk core?
...
(define-public (swap-x-for-y
  ...
  (let (
    ...
    (updated-d (get-d updated-x-balance-scaled updated-y-balance-scaled (* amplification-coefficient BPS_3) convergence-threshold))
    (caller tx-sender)
  )
    (begin
      (asserts! (is-valid-pool (get pool-id pool-data) (contract-of pool-trait)) ERR_INVALID_POOL)
...
;; ability to enable/disable single sided liquidity?
  ...
  (let (
    ...
    (begin
      ...
      (if (> x-amount-fees-liquidity u0)
        (try! (contract-call? x-token-trait transfer x-amount-fees-liquidity caller fee-address none)) ;; fee-address or protocol-address?
        false
      )
...
(define-private (fold-x-for-loop (n uint) (static-data {x: uint, c: uint, b: uint, d: uint, threshold: uint, converged: uint})) 
  (let (
    ...
    (x-denominator (- (+ (* BPS_3 current-x) current-b) current-d))
...
(define-private (fold-y-for-loop (n uint) (static-data {y: uint, c: uint, b: uint, d: uint, threshold: uint, converged: uint})) 
  (let (
    ...
    (y-denominator (- (+ (* BPS_3 current-y) current-b) current-d))
...
(define-private (fold-d-for-loop (n uint) (static-data {x-bal: uint, y-bal: uint, d: uint, ann: uint, threshold: uint, converged: uint})) 
  (let (
    ...
    (current-threshold (get threshold static-data))
    (current-converged (get converged static-data))
    (new-d-partial-x (/ (* current-d current-d-partial) (* BPS_3 current-x-balance)))
    (new-d-partial (/ (* current-d new-d-partial-x) (* BPS_3 current-y-balance)))
    (new-numerator (* (+ (* current-ann current-s) (* BPS_3 new-d-partial)) current-d))
    (new-denominator (+ (* (- current-ann BPS_4) current-d) (* (+ BPS_3 BPS_4) new-d-partial)))
